...
**Universe selectors** (all live in the `Universe(...)` declaration and persist in the DSL alongside `resolved`): `mode` (`manual` | `category` | `top_volume`), `market` (`perp` | `spot`), `symbols` (manual mode), `categories` (category mode), `top_n` (top_volume mode), `inclusions` / `exclusions` (force-include / exclude symbols), `lookback` (`7d` | `30d` | `90d` — the volume-ranking window for `top_volume`, default `7d`), `volume_quartiles` (`q1`–`q4`, where q1 = top 25% by volume; filters the volume ranking, alone or combined with `top_n`), and `groups`. These match the web editor exactly — set whichever selectors the user describes (via `universe_set` or by editing the `Universe(...)` declaration directly), then `universe_resolve` to snapshot the concrete list.
